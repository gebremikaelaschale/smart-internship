...
function simpleStem(token = '') {
  ...
  if (stemmed.endsWith('ies') && stemmed.length > 4) stemmed = `${stemmed.slice(0, -3)}y`;
  else if (stemmed.endsWith('ing') && stemmed.length > 5) stemmed = stemmed.slice(0, -3);
  else if (stemmed.endsWith('ed') && stemmed.length > 4) stemmed = stemmed.slice(0, -2);
  else if (stemmed.endsWith('ers') && stemmed.length > 5) stemmed = stemmed.slice(0, -3);
  else if (stemmed.endsWith('er') && stemmed.length > 4) stemmed = stemmed.slice(0, -2);
  else if (stemmed.endsWith('ment') && stemmed.length > 6) stemmed = stemmed.slice(0, -4);
  else if (stemmed.endsWith('tion') && stemmed.length > 6) stemmed = stemmed.slice(0, -4);
  else if (stemmed.endsWith('s') && stemmed.length > 4) stemmed = stemmed.slice(0, -1);
...
function isDepartmentCompatible(studentDepartment = '', internshipDepartment = '', targetDepartments = []) {
  ...
  if (internshipKey) return studentKey === internshipKey;
  if (targetKeys.length > 0) return targetKeys.some((key) => key === studentKey);
  return false;
}

function collectStudentSegments(profile = {}, student = {}) {
  return [
    { value: profile?.skills_description || profile?.academicInfo?.skills_description, weight: SOURCE_WEIGHTS.studentSkills * 2, source: 'skills_description' },
    { value: buildStudentVectorString(profile, student), weight: SOURCE_WEIGHTS.studentSkills * 1.5, source: 'vector' },
...
function collectInternshipSegments(internship = {}) {
  return [
    { value: internship?.internship_requirements || internship?.description, weight: SOURCE_WEIGHTS.internshipSkills * 2, source: 'internship_requirements' },
    { value: buildInternshipVectorString(internship), weight: SOURCE_WEIGHTS.internshipSkills * 1.5, source: 'vector' },
...
function buildWeightedFeatureMap(segments = []) {
  ...
  segments.forEach((segment) => {
    ...
    tokens.forEach((token) => {
      const canonical = simpleStem(token);
      if (!canonical) return;
...
function buildTopMatchingKeywords(studentSegments = [], internshipSegments = []) {
  const studentTokens = studentSegments.flatMap((segment) => tokenizeText(segment.text).map((token) => ({
    raw: token,
    canonical: simpleStem(token),
    source: segment.source,
    weight: getTokenImportance(simpleStem(token)) * Number(segment.weight || 1)
...
export function calculateInternshipMatch(internship = {}, profile = {}, options = {}) {
  return evaluateInternshipMatch(internship, profile, options);
}

export function calculateMatchScore(studentProfile = {}, internshipRequirement = {}, options = {}) {
  return evaluateInternshipMatch(internshipRequirement, studentProfile, options);
}

export function getMatchTone(score = 0) {
  if (score >= 80) {
    return {
      label: 'Strong Fit',
