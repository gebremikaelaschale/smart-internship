...
export default function DeanWelcomeHeader({
  deanName = 'Dr. Worku Kebede',
  lastSyncedAt = '',
  loading = false,
  onRefresh
}) {
  const [collegeName, setCollegeName] = useState('Assigned College');
  useEffect(() => {
    let active = true;
    ...
    const loadHeaderMeta = async () => {
      try {
        ...
        const fetchedCollege = String(profileData?.college || '').trim();
        if (fetchedCollege) {
          setCollegeName(fetchedCollege);
        }
      } catch {
        // Keep fallback metadata when request fails.
      }
    };
    ...
    <section className="rounded-[1.6rem] border border-[#E2E8F0] bg-white px-8 py-10 shadow-none">
      <div className="border-l-2 border-blue-600 pl-6">
        <div className="flex items-start justify-between gap-8">
          <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-[#64748B]">Dean Portal Overview</p>
        ...
        <div className="mt-8 max-w-4xl">
          ...
          <motion.h2
            ...
            className="mt-3 mb-3 text-5xl font-semibold tracking-tight text-[#1E293B]"
          >
            {deanName}
          </motion.h2>
          ...
          <motion.div
            ...
            className="mt-6 flex flex-wrap items-center gap-3 text-base font-medium text-[#64748B]"
          >
