...
const links = [
  ...
  { to: '/employer/post-internship', label: 'Post Internship', icon: <InternshipIcon />, accent: 'from-blue-500 to-indigo-600' },
  ...
  { to: '/employer/messages', label: 'Messages', icon: <MessageIcon />, accent: 'from-lime-500 to-emerald-600' }
];
...
export default function EmployerSidebar() {
  ...
  const [theme, setTheme] = useState(() => {
    try {
      const saved = localStorage.getItem('employer_sidebar_theme');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('employer_sidebar_theme', theme);
    } catch {
      // ignore persistence errors
    }
  }, [theme]);
      ...
      <div className="border-b border-white/10 px-5 py-5">
        <div className="flex items-start justify-between gap-2">
          <div className={collapsed ? 'hidden' : 'block'}>
            <p className={`text-xs font-semibold uppercase tracking-[0.32em] ${activeTheme.tagClass}`}>Industry Portal</p>
            <h2 className="mt-2 text-[32px] font-semibold leading-[1] tracking-tight text-white">Internship Studio</h2>
            <p className={`mt-3 text-sm leading-6 ${activeTheme.subtitleClass}`}>Manage internships, applicants, and performance in one workspace.</p>
          </div>
        ...
        {collapsed ? (
          <div className="mt-3 text-center">
            <span className="text-[10px] font-semibold uppercase tracking-[0.25em] text-white/70">IP</span>
          </div>
        ) : null}
      </div>
