...
const links = [
  { to: '/employer/dashboard', label: 'Dashboard', icon: <DashboardIcon />, accent: 'from-cyan-500 to-blue-600', softClass: 'bg-cyan-50 text-cyan-600 border border-cyan-100/30 dark:bg-cyan-950/40 dark:text-cyan-400 dark:border-cyan-900/30' },
  { to: '/employer/profile', label: 'Company Profile', icon: <CompanyIcon />, accent: 'from-amber-500 to-orange-600', softClass: 'bg-amber-50 text-amber-600 border border-amber-100/30 dark:bg-amber-950/40 dark:text-amber-400 dark:border-amber-900/30' },
  { to: '/employer/post-internship', label: 'Post Internship', icon: <BriefcaseIcon />, accent: 'from-blue-500 to-indigo-600', softClass: 'bg-blue-50 text-blue-600 border border-blue-100/30 dark:bg-blue-950/40 dark:text-blue-400 dark:border-blue-900/30' },
  { to: '/employer/my-programs', label: 'My Programs', icon: <ListIcon />, accent: 'from-violet-500 to-fuchsia-600', softClass: 'bg-violet-50 text-violet-600 border border-violet-100/30 dark:bg-violet-950/40 dark:text-violet-400 dark:border-violet-900/30' },
  { to: '/employer/applicants', label: 'Applicants', icon: <UsersIcon />, accent: 'from-teal-500 to-emerald-600', softClass: 'bg-teal-50 text-teal-600 border border-teal-100/30 dark:bg-teal-950/40 dark:text-teal-400 dark:border-teal-900/30' },
  { to: '/employer/active-interns', label: 'Active Interns', icon: <StudentIcon />, accent: 'from-emerald-500 to-teal-600', softClass: 'bg-emerald-50 text-emerald-600 border border-emerald-100/30 dark:bg-emerald-950/40 dark:text-emerald-400 dark:border-emerald-900/30' },
  { to: '/employer/logbooks', label: 'Intern Logbooks', icon: <FolderIcon />, accent: 'from-indigo-500 to-blue-600', softClass: 'bg-indigo-50 text-indigo-600 border border-indigo-100/30 dark:bg-indigo-950/40 dark:text-indigo-400 dark:border-indigo-900/30' },
  { to: '/employer/evaluation', label: 'Evaluation', icon: <ListIcon />, accent: 'from-orange-500 to-red-600', softClass: 'bg-orange-50 text-orange-600 border border-orange-100/30 dark:bg-orange-950/40 dark:text-orange-400 dark:border-orange-900/30' },
  { to: '/employer/activity', label: 'Recent Activity', icon: <ApplicationIcon />, accent: 'from-sky-500 to-cyan-600', softClass: 'bg-sky-50 text-sky-600 border border-sky-100/30 dark:bg-sky-950/40 dark:text-sky-400 dark:border-sky-900/30' },
  { to: '/employer/messages', label: 'Messages', icon: <MessageIcon />, accent: 'from-rose-500 to-pink-600', softClass: 'bg-rose-50 text-rose-600 border border-rose-100/30 dark:bg-rose-950/40 dark:text-rose-400 dark:border-rose-900/30' },
  { to: '/employer/notifications', label: 'Notifications', icon: <BellIcon />, accent: 'from-violet-500 to-purple-600', softClass: 'bg-violet-50 text-violet-600 border border-violet-100/30 dark:bg-violet-950/40 dark:text-violet-400 dark:border-violet-900/30' },
  { to: '/employer/settings', label: 'Settings', icon: <SettingsIcon />, accent: 'from-slate-600 to-slate-800', softClass: 'bg-slate-100 text-slate-600 border border-slate-200/50 dark:bg-slate-800/60 dark:text-slate-300 dark:border-slate-700/50' }
];
...
export default function EmployerSidebar() {
  ...
  useEffect(() => {
    const onKeyDown = (event) => {
      if (event.altKey && String(event.key).toLowerCase() === 'e') {
      ...
      <div className={`border-b px-6 py-6 ${isGlobalDarkMode ? 'border-slate-800' : 'border-indigo-100/70'}`}>
        <div className="flex items-start justify-between gap-2">
          <div className={collapsed ? 'hidden' : 'block'}>
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h2 className="text-[10px] font-bold tracking-[0.24em] text-amber-600 dark:text-amber-400 uppercase">Studio</h2>
              </div>
            </div>

            <div className="mt-4 flex">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-50 dark:bg-amber-950/40 px-3 py-1 text-[10px] font-bold tracking-wider text-amber-700 dark:text-amber-400 border border-amber-100/50 dark:border-amber-900/30 uppercase shadow-sm">
          ...
          <button
            ...
            title={`${collapsed ? 'Expand sidebar' : 'Collapse sidebar'} (Alt+E)`}
          >
          ...
          >
            {({ isActive }) => (
              <>
                ...
                {!collapsed && link.to === '/employer/messages' && messageUnreadCount > 0 ? (
                ...
                {!collapsed && link.to === '/employer/notifications' && notificationCount > 0 ? (
                ...
                {collapsed ? (
                  <>
                    ...
                    {link.to === '/employer/messages' && messageUnreadCount > 0 ? (
                    ...
                    {link.to === '/employer/notifications' && notificationCount > 0 ? (
